...
def show_column_position(dat,ARIAL_FONT_PATH):
    font = ImageFont.truetype(ARIAL_FONT_PATH, size=20)
...
def show_column_position_another(dat,ARIAL_FONT_PATH):
    font = ImageFont.truetype(ARIAL_FONT_PATH, size=20)
    background = np.array([[0 for _ in range(255)] for _ in range(255)], dtype='uint8')
    image = Image.fromarray(background)
    draw = ImageDraw.Draw(image)
...
def data_to_image(data, ARIAL_FONT_PATH):
    data_images = []
    font = ImageFont.truetype(ARIAL_FONT_PATH, size=50)
    for dat in data:
        background = np.array([[0 for _ in range(255)] for _ in range(255)], dtype='uint8')
        image = Image.fromarray(background)
        draw = ImageDraw.Draw(image)
...
def data_to_image_another(data,ARIAL_FONT_PATH):
    data_images = []
    font = ImageFont.truetype(ARIAL_FONT_PATH, size=50)
    for dat in data:
        background = np.array([[0 for _ in range(255)] for _ in range(255)], dtype='uint8')
